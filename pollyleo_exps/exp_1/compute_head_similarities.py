...
def compute_head_similarities(model, layer_idx):
    ...
    # Попробуем достать количество голов из конфигурации
    ...
    if num_q_heads is None:
        # Фоллбек: восстанавливаем из размеров весов
        hidden_size = q_proj.shape[0]
        head_dim = getattr(model.config, "hidden_size", hidden_size)
        num_q_heads = hidden_size // (hidden_size // num_kv_heads)

    head_dim = q_proj.shape[0] // num_q_heads  # размер одной головы

    # Q: [num_q_heads, head_dim, in_features]
    q_heads = q_proj.view(num_q_heads, head_dim, -1)
    # K: [num_kv_heads, head_dim, in_features]
